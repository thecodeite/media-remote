...
const Xbox = require('xbox-smartglass-core-node/src/xbox');
const XBox = require('./XBox');
const Events = require('xbox-smartglass-core-node/src/events');

const client_id = Math.floor(Math.random() * (999 - 1)) + 1;
var Debug = require('debug')('smartglass:client-'+client_id)

const DISCOVERY_TIMOUT = 20000
function discover(ip) {
  return new Promise(async (resolve, reject) => {
    ...
    const socketWrapper = await getSocket()

    const discovery_packet = Packer('simple.discovery_request')
    const message  = discovery_packet.pack()

    const consoles_found = []
    const complete = () => {
      socketWrapper.socket.close()
      resolve(consoles_found)
    }

    let timeoutHandle = setTimeout(complete, DISCOVERY_TIMOUT)

    socketWrapper.events.on('_on_discovery_response', function(message, remote) {
      consoles_found.push({
        message: message.packet_decoded,
        remote: remote,
        connect: async () => {
          const newXBox = new XBox(remote.address, message.certificate);
          await newXBox.connect()
          return newXBox
        }
      })
      ...
      if(!is_broadcast){
        ...
        timeoutHandle = setTimeout(complete, DISCOVERY_TIMOUT)
      }
    })
...
function getSocket()
{
  return new Promise((resolve, reject) => {
    const result = {
      ...
      events: Events(),
      send: (message, ip) => new Promise((resolve, reject) => {
        result.socket.setBroadcast(!ip);
        ip = ip || '255.255.255.255'
        result.socket.send(message, 0, message.length, 5050, ip , function(err, bytes) {
          if (err) {
            console.error(err)
            reject(err)
          }
          Debug('['+client_id+'] Sending packet to client: '+ip +':'+5050);
    ...
    Debug(`[${client_id}] Get active socket`);

    result.socket.on('listening', () => {
      resolve(result)
    })
    
    result.socket.on('error', (error) => {
      Debug('Socket Error:')
      Debug(error)
    })
    
    result.socket.on('message', (message, remote) => {
      result.last_received_time = Math.floor(Date.now() / 1000)
      Debug('['+client_id+'] Received packet at '+result.last_received_time);
      result.events.emit('receive', message, remote);
    });
    
    result.socket.on('close', () => {
      Debug(`[${client_id}] UDP socket closed`);
    });
